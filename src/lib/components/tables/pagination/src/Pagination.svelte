...
<div class="flex text-gray-700 text-{small ? 'base' : 'lg'}">
  ...
  <div
    ...
    {#each arr_pages as i}
      <div
        class="w-10 sm:flex justify-center items-center hidden select-none
        cursor-pointer leading-5 transition duration-150 ease-in {i == current ? (rounded ? ` bg-${color}-600 p-3 text-white` : `border-t-2 border-${color}-600 `) : rounded ? ' ' : 'border-t-2 border-white'}
        "
    ...
    {/each}
    <div
      class="w-10 h-10 sm:hidden flex justify-center select-none items-center 
      cursor-pointer leading-5 transition duration-150 ease-in {rounded ? ` bg-${color}-600 text-white` : `border-t-2 border-${color}-600`}">
      {current}
    </div>
  </div>
  <div
