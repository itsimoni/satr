<script lang="ts">
	...
	function filterData(data: any) {
		...
		if (searchInput !== "") {
			const uniqueData = [
				...data
					.reduce((map, obj) => map.set(obj.change, obj), new Map())
					.values(),
			];
			return uniqueData;
		} else {
...
<div class="border-borderColor bg-mainbg lg:w-1/4 lg:border-r">
	<div class="px-2 lg:pt-2">
		<div class="group relative flex items-center transition-all delay-75">
			<input
				type="text"
				class="inputInitial transition delay-75 hover:placeholder:text-[#63687A]"
				placeholder="Search"
			...
			<div class="group absolute right-2 flex h-6 w-6 items-center">
				<svg
					...
					class="h-4 w-4 stroke-[#63687A] transition delay-75  hover:stroke-white group-focus-within:stroke-white group-focus:stroke-white group-active:stroke-white"
					fill="none"
					viewBox="0 0 24 24"
					stroke="currentColor"
					stroke-width="2"
		...
		<div class="w-full">
			...
			<div
				class="noScroll h-[22vh] border-b  overflow-auto border-borderColor text-white lg:h-[81vh]"
			>
				<table
					id="pairsLeft"
					class="relative m-0 w-full table-fixed border-borderColor text-sm  {searchInput
						? 'border-b'
						: ''} border-borderColor"
				>
					<tbody class="noScroll overflow-auto text-xs">
						{#await createPairs(active, searchInput)}
							{#key $locale}
								<tr
							...
							{/key}
						{:then data}
							{#each filterData(data) as { symbol, value, change, primary }, i}
								<tr
									on:click={() => {
